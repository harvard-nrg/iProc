...
#    srun -n 1 -c $SLURM_CPUS_PER_TASK 3dDespike -NEW -prefix ${RESID_IN%.nii.gz}_despike -overwrite -ssave ${DESPIKE_OUT%.nii.gz}_spikiness -q ${RESID_IN}
#else
3dDespike -NEW -prefix ${RESID_IN%.nii.gz}_despike -overwrite -ssave ${DESPIKE_OUT%.nii.gz}_spikiness -q ${RESID_IN}
#fi

3dAFNItoNIFTI -prefix ${DESPIKE_OUT%.nii.gz} -overwrite ${DESPIKE_OUT%.nii.gz}+tlrc.BRIK -float
gzip -f ${DESPIKE_OUT%.nii.gz}.nii

3dAFNItoNIFTI -prefix ${DESPIKE_OUT%.nii.gz}_spikiness -overwrite ${DESPIKE_OUT%.nii.gz}_spikiness+tlrc.BRIK -float
gzip -f ${DESPIKE_OUT%.nii.gz}_spikiness.nii


#rm ${DESPIKE_OUT%.nii.gz}.nii
#rm ${DESPIKE_OUT%.nii.gz}_spikiness.nii
...
#rsync -av ${SCRATCHDIR}/* ${OUTDIR}
rm -r ${SCRATCHDIR}

rm ${DESPIKE_OUT%.nii.gz}*+tlrc.BRIK
rm ${DESPIKE_OUT%.nii.gz}*+tlrc.HEAD
